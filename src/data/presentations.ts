...
const presentationsFixture: Array<PresentationItem> = [
  {
    id: "1",
...
export const listPresentations = createServerFn({
  method: "GET",
}).handler(() => presentationsFixture);

export const getPresentationById = createServerFn({
  method: "GET",
})
  .inputValidator((data: { id: PresentationItem["id"] }) => data)
  .handler(({ data }) => presentationsFixture.find((e) => e.id === data.id));
